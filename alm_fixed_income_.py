...
INPUT_FILENAMES = {
    ...
    "usd_swaps": "USD SWAP CURVE 1-30yr (1).xlsx",
...
def resolve_input_file(filename: str) -> Path:
    """Find an input workbook in the project root first, then data/."""

    candidates = [PROJECT_ROOT / filename, DATA_DIR / filename]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        f"Required input file not found: {filename}. Checked: "
